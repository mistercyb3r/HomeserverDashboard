...
export function StorageSection({ mounts }: StorageSectionProps) {
  return (
    <section className="px-4 sm:px-6 lg:px-8">
      ...
      {!mounts.length ? (
        <p className="rounded-xl border border-border/70 bg-surface/40 px-3.5 py-3 text-sm text-faint">
          No host filesystems detected. In Docker, mount the host root at{" "}
          <span className="font-mono text-muted">/hostfs</span> (see compose).
        </p>
      ) : (
